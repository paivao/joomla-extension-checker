...
class FeedItem(NamedTuple):
    ...
    status: int|None = None  # 1 = live, 2 = resolved
    jed: str|None = None
    cve_id: str|None = None
    cwe_id: str|None = None
    risk_level: str|None = None
    recommendation: str|None = None
    cvss30_base: str|None = None
    cvss30_base_score: str|None = None
    start_version: str|None = None
    vulnerable_version: str|None = None
    patch_version: str|None = None
    update_notice: str|None = None
    install_data: str|None = None
...
class CVEEntry(NamedTuple):
    ...
    def from_api(cls, data: dict[str, Any]):
        ...
        data.update(
            {
                "description": next(
                    filter(lambda x: x["lang"] == "en", descriptions), descriptions[0]
                )["value"],
                "published": datetime.fromisoformat(
                    data.pop("published").replace("Z", "+00:00")
                ),
                "lastModified": datetime.fromisoformat(
                    data.pop("lastModified").replace("Z", "+00:00")
                ),
            }
        )
        return cls(**data)
    ...
    def from_db(cls, row: sqlite3.Row):
        return cls(
            *(
                row[:2]
                + tuple(datetime.fromisoformat(i) for i in row[2:4])
                + row[4:6]
                + (json.loads(row[6]),)
                + row[7:14]
...
class NVDAPIResponse(NamedTuple):
    ...
    def from_api(cls, data: dict[str, Any]):
        ...
        vulnerabilities = [
            CVEEntry.from_api(entry["cve"]) for entry in data.pop("vulnerabilities")
        ]
